...
| TASK-074 | ⏳ | TUIのインライン描画方針（ANSIベース）に移行する | TASK-073 |
| TASK-075 | ⏳ | TUIでLLM応答のストリーミング表示を実装する | TASK-074 |
| TASK-076 | ⏳ | 入力エリアを改行数に応じて可変高さにする | TASK-074 |
| TASK-077 | ⏳ | 処理ステータス表示を入力エリア内に統合する | TASK-074 |
| TASK-078 | ⏳ | 結果エリアの拡張ルール（5行未満に縮小しない）を実装する | TASK-074 |
| TASK-079 | ⏳ | フッターをANSIで固定再描画する（実現不可時は入力近傍に表示） | TASK-074 |
| TASK-080 | ⏳ | ツール実行許可UI（y/n/a/d）を実装する | TASK-074 |
...
| TASK-085 | ⏳ | Todoリスト表示UIを実装する | TASK-074 |
| TASK-086 | ⏳ | エラー表示の色分け・折りたたみ仕様を実装する | TASK-074 |
