...
| --- | --- | --- | --- | --- | --- |
| Core CLI modes and formats | 1. Core Functional Requirements | Must | Complete | ALT-014,ALT-015 | TUI, headless `-p`, `json`, real `stream-json`, `--cwd`, `--add-dir`, and startup tool controls are wired into the execution paths. |
...
| File references and completion | 9. File Operations | Must | Complete | ALT-014,ALT-021,ALT-030 | Basic file and image attachment paths, TUI `@` completion, fuzzy matching, recent file history, and `.gitignore` handling are implemented with tests. |
...
| GitHub/GitLab integration | 10.2 GitHub/GitLab Integration | Optional | Partial | ALT-014,ALT-022,ALT-031 | GitHub/GitLab issue, PR/MR, label, comment, and review commands are implemented through `gh` / `glab` wrappers. Live external-service E2E remains opt-in future evidence. |
...
| Authentication | 12. Authentication | Must | Complete | ALT-014,ALT-023,ALT-032 | API-key env status, login/logout/status, encrypted token storage, and stored token hydration are implemented. OAuth remains optional because the tracked task allowed OAuth or encrypted storage. |
...
| Advanced features | 14. Optional Features | Optional | Partial | ALT-014,ALT-033,ALT-034 | Review mode, checkpoints, structured project memory search, usage aggregation, and cloud LLM inference through providers exist. Broader semantic knowledge base indexing remains optional future work. |
...
| Testing requirements | 17. Testing | Must | Partial | ALT-014,ALT-022,ALT-026,ALT-028,ALT-029,ALT-035 | `cargo test` passes the current suite, including integration-style Git/review tests and binary-level E2E tests for auth, sessions, agent, MCP, tools, perf, large-repository file operations, and long-session persistence. CI-gated core coverage requires every in-scope file to be at least 90% line covered, excluding the CLI dispatcher, interactive TUI, and network adapters; full interactive TUI/E2E coverage still requires more evidence. |
...
| Security requirements | 20. Security | Must | Partial | ALT-014,ALT-017,ALT-018,ALT-023,ALT-032 | Approval and sandbox basics, default `.env` denial, configurable sensitive path blocks, filtered child env, dangerous-command blocking, opt-in JSONL audit logs, and encrypted token storage exist. Broader sandbox and secret-detection evidence remains future work. |
