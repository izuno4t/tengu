...
- 応答本文は `{"type":"response", ...}` で出力する
- プロバイダが usage を返した場合は、その前に `{"type":"usage", ...}` を追加出力する
- usage がない場合は `response` のみを出力する

**`json` イベント例:**

```json
{"type":"usage","usage":{"provider":"anthropic","input_tokens":128,"output_tokens":32,"total_tokens":160}}
{"type":"response","mode":"llm","prompt":"Reply with OK","message":"OK"}
```
...
{"type":"usage","mode":"llm","usage":{"provider":"openai","input_tokens":128,"output_tokens":32,"total_tokens":160}}
